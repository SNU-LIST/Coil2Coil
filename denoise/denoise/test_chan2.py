...
cd /home/juhyung/temp/denoise/denoise
python test_chan2.py --gpu 1
...
def parse():
    ...
    ### data & directory ###
    parser.add_argument("--model", default = '/home/juhyung/gate/denoise/denoise/model_complex.training') # model path
    parser.add_argument("--dataset","-d", default = '/home/juhyung/data/sooyeonData/highresGRE/beforedenoising.mat')  # data path
    parser.add_argument("--run_dir","-dir", default = '/home/juhyung/gate/denoise/denoise/log/') # output path
    parser.add_argument("--mat_key", default = 'measc') # data key
...
class DataWrapper(Dataset):
    ...
    def __init__(self, 
        ...
        img_real = torch.from_numpy(self.files_raw.real).type(torch.float).unsqueeze(0)
        img_imag = torch.from_numpy(self.files_raw.imag).type(torch.float).unsqueeze(0)        
        self.files_raw = torch.cat((img_real,img_imag),dim=0)
        #self.files_raw = torch.from_numpy(self.files_raw).type(torch.float)
        
        self.file_size = self.files_raw.shape

        if len(self.file_size) == 4:
            self.max_int = abs(self.files_raw).max()
            self.files_raw = (self.files_raw) / (self.max_int)
        elif len(self.file_size) == 5:
            files_view = self.files_raw.reshape(-1,self.file_size[-1])
            self.max_int = abs(files_view).max(0).values
            self.files_raw = (self.files_raw) / (self.max_int)

            self.files_raw = self.files_raw.reshape([self.file_size[0],self.file_size[1],self.file_size[2],-1])
        else:
            raise('cannot input except 4 or 5 dim')
        
        self.img_count = self.files_raw.shape[3]
...
class Evaluator:
    def __init__(self, args):
        ...
        self.network = Network.from_state_dict(state_dict,logger,chan_num=2)
    ...
    def evaluate(self):
        ...
        if len(self.testdataset.file_size) == 5:
            result_inp = result_inp.reshape(self.testdataset.file_size)
            result_out = result_out.reshape(self.testdataset.file_size)

        result_inp = result_inp * (self.testdataset.max_int) 
        result_out = result_out * (self.testdataset.max_int)
        
        result_inp = result_inp.numpy()
        result_out = result_out.numpy()  
        result_inp = result_inp[0,...] + 1j * result_inp[1,...]
        result_out = result_out[0,...] + 1j * result_out[1,...]
